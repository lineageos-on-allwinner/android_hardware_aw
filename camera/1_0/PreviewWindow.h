...
public:
    ...
    void deallocate(int cnt);

    int lockBuffer(int index);

    int unlockBuffer(int index);
...
public:
    ...
    //struct private_handle_t *mPrivateHandle[NB_BUFFER];     //for A33 GPU other
    private_handle_t* mPrivateHandle[NB_BUFFER]; //for A83 GPU
...
protected:
    ...
    //struct AwCameraMemInfo mMemInfo[NB_BUFFER];
    int mBufferFlag[NB_BUFFER];
};
