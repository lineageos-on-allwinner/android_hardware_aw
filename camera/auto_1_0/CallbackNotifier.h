...
enum ReleaseBuffer
{
    NEED_RELEASE_INDEX = 0,              //release hal buffer by index in framework
    NOT_NEED_RELEASE_INDEX = 0xEFE,      //hal manage buffer by self
    RELEASE_INDEX_MAX
};
...
public:
     ...
     */
 #if ENABLE_CAMERA_SOURCE
    void onNextFrameAvailable(const void* frame,bool hw, int release);
    void onNextFrameAvailable(const void** frame, int count, int release);
    void onNextFrameHW(const void** frame, int count, int release);
 #else
