...
static void __isp_ctx_cfg_mod(struct isp_lib_context *isp_gen)
{
	...
	if (param->isp_test_settings.msc_en) {
		...
		memcpy(mod_cfg->msc_cfg.msc_blw_lut, param->isp_tunning_settings.msc_blw_lut,
			sizeof(mod_cfg->msc_cfg.msc_blw_lut));
		memcpy(mod_cfg->msc_cfg.msc_blh_lut, param->isp_tunning_settings.msc_blh_lut,
			sizeof(mod_cfg->msc_cfg.msc_blh_lut));
		...
		if(param->isp_tunning_settings.msc_mode >= 4) {
			...
			mod_cfg->msc_cfg.msc_blw_dlt_lut[11] = clamp(4096/(mod_cfg->msc_cfg.msc_blw_lut[10] +1), 0, 4095);
			for(i = 1; i < 11; i++) {
			...
			mod_cfg->msc_cfg.msc_blh_dlt_lut[11] = clamp(4096/(mod_cfg->msc_cfg.msc_blh_lut[10] +1), 0, 4095);
			for(i = 1; i < 11; i++) {
