    ...
    ,mFastPictureMode(false)
{
    ...
    if (mhKeyFile == (void *)-1)
    {
        HAL_LOGE("open file %s failed", CAMERA_KEY_CONFIG_PATH);
        return;
    }
    else
    {
        HAL_LOGV("open file %s OK", CAMERA_KEY_CONFIG_PATH);
    }

    // exif
    readKey(kCAMERA_EXIF_MAKE, mCameraMake);
    readKey(kCAMERA_EXIF_MODEL, mCameraModel);
    HAL_LOGV("exif, make: %s, model: %s", mCameraMake, mCameraModel);
    ...
    if(readKey(kNUMBER_OF_CAMERA, numberOfCamera))
    {
        ...
        HAL_LOGV("read number: %d", mNumberOfCamera);
    }

    if (mCurCameraId + 1 > mNumberOfCamera)
    {
        HAL_LOGV("should not read camera id: %d, there are %d camera(s)", mCurCameraId, mNumberOfCamera);
        return;
    }
...
bool CCameraConfig::readKey(char *key, char *value)
{
    ...
    if (key == 0 || value == 0)
    {
        HAL_LOGV("error input para");
        return false;
    }

    if (mhKeyFile == 0)
    {
        HAL_LOGV("error key file handle");
        return false;
    }
