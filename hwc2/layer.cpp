...
deal:
	...
	memset(submitLayer->traceName, 0, sizeof(submitLayer->traceName));

	return submitLayer;
}

void incRef(Layer_t *layer)
{
    pthread_mutex_lock(&chaceMutex);
    layer->ref++;
    pthread_mutex_unlock(&chaceMutex);
...
void layerCachePut(Layer_t *layer)
{
	...
	if (layer->myselfHandle) {
		...
		if (layer->buffer != NULL && handle->metadata_fd >= 0)
			close(handle->metadata_fd);
#endif
		layer->myselfHandle = 0;
...
#if (GRALLOC_SUNXI_METADATA_BUF & (DE_VERSION == 30))
		handle2->metadata_fd = dup(handle->metadata_fd);
#endif
		duplayer->buffer = (buffer_handle_t)handle2;
	}
