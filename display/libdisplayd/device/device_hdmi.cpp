...
HdmiDevice::HdmiDevice(sunxi::IHWCPrivateService& client)
  : DeviceControler(DISP_OUTPUT_TYPE_HDMI, client),
...
int HdmiDevice::onDataspaceChange(int dataspace)
{
    ...
    int transfer = dataspace & HAL_DATASPACE_TRANSFER_MASK;
    ...
    if (mEdidStrategy.setupConfig(request, &config, &dirty)) {
        ...
        int type = request == EdidStrategy::HDR ? DATASPACE_HDR : DATASPACE_SDR;
        hwcSetDataspace(type);
...
int HdmiDevice::performOptimalMode()
{
    ...
    if (mDeviceConfig.get().type != DISP_OUTPUT_TYPE_HDMI) {
        savedmode = DEFAULT_HDMI_OUTPUT_MODE;
        dd_info("performOptimalMode: saved mode is invalid, use default mode %d",
                savedmode);
    }
...
{
    mEdidStrategy.update();
    int vendor = mEdidStrategy.getNative();
