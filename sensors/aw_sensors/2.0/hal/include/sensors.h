...
#define LSG_MC32X0                      (1024.0f)
#define LSG_LIS3DE_ACC                  (65.0f)
#define LSG_BMA250                      (1024.0f)
...
struct sensors_poll_context_t {
    sensors_poll_device_2_t device; // must be first

    sensors_poll_context_t();
    ~sensors_poll_context_t();
    int activate(int handle, int enabled);
    int setDelay(int handle, int64_t ns);
    int injectSensorData(int handle, const Event &event);
    void setOperationMode(OperationMode mode);
    SensorBase* getSensor(int index) {return mSensors[index];}
    int flush(int sensor_handle);
    Result setCallback(const ::android::hardware::MQDescriptorSync<Event>& eventQueueDescriptor,
...
extern int seSensorIndex[SUPPORT_SENSORS_NUMBER];
extern int sNumber;
