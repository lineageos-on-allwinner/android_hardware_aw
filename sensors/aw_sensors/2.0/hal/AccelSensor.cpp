...
#define ACC_REVISE_TIME   2     //the revise time more larger, the sensor respond more slow
//#define DEBUG_SENSOR 1

static float LastFiltered[3];
static float lastData[3];
static int lastRevise;
static bool UseRevise;
    ...
    mDelay(0)
{
    ...
    if (!strcmp(ACC_DATA_NAME, "sc7660")) {
        UseRevise = true;
    } else {
        UseRevise = false;
    }
...
int AccelSensor::gsensor_cfg()
{
    ...
    property_get("ro.vendor.sf.rotation", property, 0);
...
int AccelSensor::setEnable(int32_t handle, int en) {
    ...
    ALOGD("acc sensor setEnable:  handle:  %d, en: %d", handle, en);
...
int AccelSensor::setDelay(int32_t handle, int64_t ns) {
    ALOGD("delay:  handle:  %d, ns: %" SCNi64, handle, ns);
    if (ns < 0)
        return -EINVAL;
...
bool AccelSensor::ReviseEvent() {
    float tmpx, tmpy, tmpz;
    tmpx = mPendingEvent.acceleration.x;
    tmpy = mPendingEvent.acceleration.y;
    tmpz = mPendingEvent.acceleration.z;
    float total = (float) sqrt(tmpx * tmpx + tmpy * tmpy + tmpz * tmpz);
    if (total > 1.5 * GRAVITY_EARTH && lastRevise < ACC_REVISE_TIME) {
        mPendingEvent.acceleration.x = lastData[0];
        mPendingEvent.acceleration.y = lastData[1];
        mPendingEvent.acceleration.z = lastData[2];
        lastRevise++;
        ALOGD("sensor AccelSensor has a larger change, first time drop");
        if (lastTotal < 1) return false;
    } else if (total > 0.8 * GRAVITY_EARTH && total < 1.2 * GRAVITY_EARTH) {
        lastRevise = 0;
    }
    lastData[0] = mPendingEvent.acceleration.x;
    lastData[1] = mPendingEvent.acceleration.y;
    lastData[2] = mPendingEvent.acceleration.z;
    lastTotal = total;
    return true;
}
...
int AccelSensor::writeDelay(int64_t ns) {
    ...
    if (ns < 10000000LL) {
        ns = 10000000LL; /* minimum delay in nano second. */
    }
    char buf[80];
    int bytes = sprintf(buf, "%" SCNi64, ns/1000 / 1000);
